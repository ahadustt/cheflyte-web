...
- [2025-05-23] CSS variables dark mode fix: Dark mode CSS custom properties had incorrect color values (--color-bg-primary was set to blue #2563eb instead of proper background color), causing button text to be invisible due to poor contrast. Fixed by updating all dark mode variables to proper neutral colors and switching button components from CSS variables to reliable Tailwind color classes with explicit dark mode variants for guaranteed visibility and contrast
- [2025-05-23] StatsSection gradient backgrounds: When using gradient backgrounds via className, text needs to be white for proper contrast. Added gradient detection logic to conditionally apply white text (text-white, text-white/90, text-white/80) instead of default dark text on light backgrounds
- [2025-05-23] CallToAction component variant conflicts: Don't override component backgrounds with className when using specific variants - CallToAction with variant="gradient" expects dark gradient background with white text, but className="bg-gray-50" override created white text on light background making it invisible. Always let variant styles take precedence over className overrides
